...
public:
    ...
    void setup() override {
        ...
        // ---- Load latent model JSON (decoder + PE + domain + Z) ----
        if (!m_trainer.loadModelJSON(m_snapshotPath)) {
            // Fallback init so sketch still runs
            m_trainer.initialize(128, 128, -1.2f, 1.2f, -1.2f, 1.2f,
                                 /*numShapes*/4, /*latentDim*/16, /*hidden*/{64,64,64}, /*seed*/1234);
        }
        m_domain = m_trainer.getDomain();

        // ---- Hook viewer just for panel rendering + picking ----
        m_viewer
            .setDomain(&m_domain)
            .setLatents(&m_trainer.getLatentCodes())
            .setDecoder([this](const std::vector<float>& z, GridField& out){
                return m_trainer.getFieldFromLatent(z, out);
            })
            .setFastDecodeHooks(
                // enc(x,y)->coord enc
                [this](float x, float y, float* out){
                    m_trainer.ad.enc.encode(x, y, m_trainer.ad.encBuf);
                    for (int j=0;j<m_trainer.ad.coordEncDim;++j) out[j] = m_trainer.ad.encBuf[j];
                },
                // single forward
                [this](const std::vector<float>& in){
                    return m_trainer.ad.decoder.forward(in);
                },
                m_trainer.ad.coordEncDim
            );

        // choose 4 corners from first codes
        const auto& Z = m_trainer.getLatentCodes();
        ...
        m_viewer.setInterpCorners(z00,z10,z01,z11);
        ...
        // Column tools (kept)
        m_ui->addSlider("Sample Dist",    Vec2{10, 320}, 240.0f, 1.0f, 20.0f, m_sampleDistance);
        m_ui->addSlider("Column Offset",  Vec2{10, 350}, 240.0f, -5.0f, 0.0f, m_columnOffset);
        m_ui->addSlider("Gravity Weight", Vec2{10, 380}, 240.0f, 0.0f, 1.0f, m_gravityWeight);
        m_ui->addToggle("Init Columns",   UIRect{10, 410, 120, 22}, m_btnInitColumns);
        m_ui->addToggle("Run Columns",    UIRect{10, 440, 120, 22}, m_makeColumns);

        // Export
        m_ui->addToggle("Export",         UIRect{10, 500, 100, 22}, m_btnExport);

        // Init status
        m_statusMessage = "Loaded latent model. Click the panel to add slices. Press G to generate stack.";

        // Cache panel once for snappy draw
        // m_viewer.drawPanel(m_dummyRenderer, m_panelN); // no-op, just to warm ensure build in real draw
    }

    void update(float) override {
        postUpdateUI();

        if (!m_makeColumns) return;
        if (m_columnParticles.empty()) { initColumnParticles(); initColumnLines(); }
        if (!m_volumeField) return;
        ...
        for (size_t i = 0; i < m_columnParticles.size(); ++i) {
            auto& p = m_columnParticles[i];
            auto& hist = m_columnTrajectories[i];
            ...
            if (!graph) {
                ...
                graph->setColor(Color(1,1,1));
                graph->setEdgeWidth(2.0f);
                scene().addObject(graph);
            }
            if (hist.empty()) {
                hist.emplace_back(p.x, p.y, p.z + 0.1f);
                hist.emplace_back(p);
                refreshColumnGraph(i);
            }

            Vec3 gravity(0,0,-1);
            Vec3 pull = m_volumeField->project_onto_isosurface(p, m_columnOffset) - p;
            Vec3 force = gravity * m_gravityWeight + pull * (1.f - m_gravityWeight);

            float vertical = force.dot(gravity);
            if (vertical < 0.f) force -= gravity * vertical;

            Vec3 step = force * m_particleStep;
            if (step.lengthSquared() <= 1e-8f) continue;

            Vec3 nextP = p + step;
            if (m_volumeField->value_at(nextP) > 0.f)
                nextP = m_volumeField->project_onto_isosurface(p, m_columnOffset);
            if (nextP.z < 0.f) nextP.z = 0.f;

            if (!hist.empty()) {
                Vec3& last = hist.back();
                if ((nextP - last).lengthSquared() < 1e-6f) {
                    p = nextP; last = nextP; refreshColumnGraph(i); continue;
                }
            }
            p = nextP;
            hist.push_back(nextP);
            refreshColumnGraph(i);
        }
    }
    ...
    bool onMousePress(int button, int state, int x, int y) override {
        ...
        auto uv = m_viewer.panelUVFromMouse((float)x,(float)y,m_panelN);
        if (!uv) return false;
    ...
    bool onKeyPress(unsigned char key, int, int) override {
        ...
        if (key=='s' || key=='S') { m_trainer.saveModelJSON(m_snapshotPath); m_statusMessage="Saved model JSON."; return true; }
...
private:
    ...
    bool m_viewerDecode(const std::pair<float,float>& uv, GridField& out) {
        return m_viewer.decodeInterpolatedField(uv.first, uv.second, out);
    }
    ...
    void generateInterpolatedStack() {
        ...
        GridField tmp;
        for (auto& uv : samples) {
            if (!m_viewerDecode(uv, tmp)) continue;
    ...
    void clearAllSelections() {
        ...
        // clear columns
        for (auto& g : m_columnLines) if (g) scene().removeObject(g);
        m_columnLines.clear();
    ...
    void exportAll() {
        ...
        // Merge columns into one graph and export both OBJ files
        auto merged = std::make_shared<GraphObject>("ExportColumns");
        for (auto& g : m_columnLines) if (g) merged->combineWith(*g);
        merged->weld();

        m_meshObject->writeToObj(m_outputMeshName);
        merged->writeToObj(m_outputGraphName);
        m_statusMessage = "Exported mesh -> " + m_outputMeshName + " & columns -> " + m_outputGraphName;
    }
    ...
    // ===== Column particles (kept) =====
    void initColumnParticles() {
        m_columnParticles.clear();
        m_columnTrajectories.clear();

        if (m_fields.empty()) return;
        auto lastContour = m_fields.back().get_contours(m_columnOffset);
        lastContour.weld();
        auto separated = lastContour.separate();

        for (auto graph : separated) {
            int sampleCount = std::max(1, int(graph.getLength() / m_sampleDistance));
            graph.resampleByCount(sampleCount);
            auto verts = graph.getGraphData()->vertices;

            for (auto& v : verts) {
                Vec3 pos(v.position.x, v.position.y, float(m_contours.size()-1)*m_sliceSpacing);
                m_columnParticles.push_back(pos);

                std::vector<Vec3> traj;
                traj.emplace_back(v.position.x, v.position.y, pos.z+0.1f);
                traj.emplace_back(pos);
                m_columnTrajectories.emplace_back(std::move(traj));
            }
        }
    }
    ...
    void initColumnLines() {
        ...
        for (size_t i=0;i<m_columnParticles.size();++i) {
            auto cg = std::make_shared<GraphObject>("StackColumn_" + std::to_string(i));
            cg->setShowVertices(false);
            cg->setShowEdges(true);
            cg->setEdgeColor(Color(0,0,0));
            cg->setEdgeWidth(4.0f);
            scene().addObject(cg);
            m_columnLines.emplace_back(std::move(cg));
            refreshColumnGraph(i);
        }
    }

    void refreshColumnGraph(size_t idx) {
        if (idx >= m_columnLines.size() || idx >= m_columnTrajectories.size()) return;
        auto& graph = m_columnLines[idx]; if (!graph) return;
        const auto& traj = m_columnTrajectories[idx]; if (traj.empty()) return;

        std::vector<std::pair<int,int>> edges;
        if (traj.size()>1) {
            edges.reserve(traj.size()-1);
            for (size_t j=1;j<traj.size();++j) edges.emplace_back(int(j-1), int(j));
        }
        graph->createFromPositionsAndEdges(traj, edges);
        graph->setShowVertices(false);
        graph->setShowEdges(true);
    }
...
private:
    // ---- Latent model + panel ----
    LatentSDF_CPU m_trainer;
    FieldDomain   m_domain;
    FieldViewer   m_viewer;
    int           m_panelN = 5;
    ...
    std::string m_outputGraphName = "outColumns.obj";
